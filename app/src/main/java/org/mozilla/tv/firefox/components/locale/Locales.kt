 ...
 * subclasses.
 *
 * Either call [initializeLocale] in your `onCreate` method, or inherit from
 * `LocaleAwareAppCompatActivity` or friends.
 */
object Locales {
     ...
     *
     * This method is equivalent to the first part of [getLanguageTag].
     *
    ...
    fun parseLocaleCode(localeCode: String): Locale {
        var index = localeCode.indexOf('-')
        if (index == -1) {
            index = localeCode.indexOf('_')
        }
        if (index != -1) {
